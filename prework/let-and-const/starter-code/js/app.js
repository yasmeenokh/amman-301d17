...
function displayPics(){
  ...
  // PUT YOUR RESPONSE IN THIS COMMENT: Variables declared by const have their scope in the block for which they are declared, as well as in any contained sub-blocks. ie the console was called outside the scope of the variable.
  console.log(viewed);
...
function handleClick(event) {
  ...
  for(let i = 0; i < names.length; i++){
    if(event.target.id === allProducts[i].name) {
      ...
      console.log(`${event.target.id} has ${allProducts[i].votes} votes in ${allProducts.views} views`);
    }
  }
